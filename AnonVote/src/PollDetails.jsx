...
export default function PollDetails() {
  ...
  useEffect(() => {
    // Get poll data from navigation state
    if (location.state?.pollData) {
      setPollData(location.state.pollData);
    } else {
      // If no poll data, redirect to create page
      navigate('/create');
    }
      ...
      <div className="flex min-h-screen">
        ...
        <div className="flex-1 ml-0 md:ml-64 p-4 md:p-10">
          <div className="flex flex-col items-center justify-center h-64">
            <h1 className="text-xl font-medium mb-4">Loading poll...</h1>
            <p className="text-gray-600">If this persists, please create a new poll.</p>
          </div>
        </div>
      </div>
    );
